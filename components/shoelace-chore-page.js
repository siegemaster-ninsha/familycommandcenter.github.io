...
const ShoelaceChorePage = Vue.defineComponent({
  ...
  methods: {
    ...
    toggleSelectionMode() { this.selectionMode = !this.selectionMode; if (!this.selectionMode) this.selectedChores.clear(); },
    selectChore(chore, event) {
      if (event && (event.type === 'touchend' || event.type === 'touchstart')) { event.preventDefault(); event.stopPropagation(); }
      ...
      } finally { setTimeout(() => { chore.isSelecting = false; }, 100); }
    },
    selectQuicklistChore(quickChore, event) {
      ...
      } finally { setTimeout(() => { quickChore.isSelecting = false; }, 100); }
    },
    openAddToQuicklistModal() { this.$parent?.openAddToQuicklistModal?.(); },
    openAddChoreModal() { this.$parent?.openAddChoreModal?.(); },
    async loadQuicklistChores() {
      this.quicklistLoading = true; this.quicklistError = null;
      try { await this.$parent.loadQuicklistChores(); }
      catch (error) { this.quicklistError = error.message; }
      finally { this.quicklistLoading = false; }
    },
    async removeFromQuicklist(quicklistId) {
      try {
        await this.$parent.apiCall(CONFIG.API.ENDPOINTS.QUICKLIST + '/' + quicklistId, { method: 'DELETE' });
        await this.loadQuicklistChores();
      } catch (error) { console.error('Failed to remove from quicklist:', error); }
    },
    ...
    async deleteChore(chore) { if (chore) await this.$parent.deleteChore(chore); },
    async assignSelectedChore(assignTo) { await this.$parent.assignSelectedChore(assignTo); },
    async handleChoreCompletionToggle(chore, event) {
      if (event) { event.preventDefault(); event.stopPropagation(); }
      chore.completed = !chore.completed;
      await this.$parent.handleChoreCompletion(chore);
    },
    getCategoryIcon(category) { return this.Helpers?.getCategoryIcon?.(category) || ''; },
    getCategoryLabel(category) { return this.Helpers?.getCategoryLabel?.(category) || ''; },
